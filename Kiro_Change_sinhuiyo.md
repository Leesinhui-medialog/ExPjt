...
### 91. Steering 파일 타입별 분리 (Java / XML·설정)
...
### 93. 메소드 네이밍 규칙 추가 (list, listForPage)
...
### 101. 약관동의 LayerPopup을 alert 형태로 변경
...
### 102. Step 01 버튼 가운데 정렬 + 취소 시 체크박스 해제
...
### 104. Step 02 이름/생년월일 입력 필드 간격 확대
...
### 106. Step 02 가운데 정렬 + label/input 간격 조정
...
### 107. Step 02 label과 input 한 라인 배치
...
### 133. JSESSIONID 기반 로그인 세션 관리 구현
...
### 138. 게시판 목록 내용 제거 + 작성자/읽음 컬럼 추가 + 제목 60%
...
### 140. BoardEdit 수정 버튼 클릭 시 로딩바 동작 확인
...
### 143. BoardEdit 로딩 텍스트 제거 + 버튼 중앙 정렬
...
### 149. GitHub pull 시 remote ref 오류 해결 안내
...
### 151. GitHub remote ref 오류 재발 해결 안내
...
### 154. GitHub push 실패 원인 분석 및 .gitignore 수정
...
### 159. Git commit 시 특정 단어 포함 라인 추출 방법 안내
...
### 161. Kiro 생성 코드 리포트 Hook 중복 파일 처리 추가
...
### 164. Kiro 생성 코드 리포트 Hook 합산 기능 추가
...
### 169. MemberEdit 로그인 정보 기반 데이터 채움 확인 + 서버 재시작
...
### 170. MemberEdit 생년월일 변경 가능 + 달력 선택
...
### 171. Kiro 생성 코드 리포트 Hook 실행 및 commit 방법 안내
...
### 177. @author Unknown → sinhuiyo 일괄 변경
...
### 178. Kiro 생성 코드 리포트 Hook에 설명 컬럼 추가
...
### 189. ExPjt_개발가이드.docx에 Git Repository 정보 추가
...
### 192. kiro-generated-state.md 작성자별 원형 그래프 추가
...
### 193. 메일 비밀번호 환경 변수 설정 및 steering 추가
...
### 203. BoardService 메일 발송 로직 분리 (리팩토링)
...
### 205. Java 변경 시 JUnit 자동 테스트 Hook 등록
...
### 206. BoardMailNotificationService → 공통 MailNotificationService로 리팩토링
...
### 207. change.md → Kiro_Change_sinhuiyo.md 파일 생성 및 change log Hook 변경
...
### 208. Kiro_Change_sinhuiyo.md 일련번호 정리

| 질의 | Kiro_Change_sinhuiyo.md의 일련번호가 맞지 않아 정리 요청 |
|------|--------------------------------------------------|
| 결과 | 원본 `change.md`에서 재복사 후 PowerShell 스크립트로 중복 항목 제거 및 일련번호 재매기기 실행. 39번 중복(38번 내용 반복), 91번 중복(90번과 동일), 99번 중복, 198번 이후 89~103번 재등장 등 문제 해결. 최종 1~208번까지 연속 일련번호로 정리 완료. |


### 209. fileEdited Hook으로 사용자 직접 수정 파일에도 Hook 적용 가능 여부 확인

| 질의 | 사용자가 임의로 변경한 파일을 저장할 때도 Hook 적용이 가능한가? |
|------|-----------------------------------------------------|
| 결과 | 가능함. Kiro Hook의 `fileEdited` 이벤트는 사용자가 IDE에서 직접 파일을 수정하고 저장(Ctrl+S)할 때도 트리거됨. Kiro 에이전트가 수정한 경우뿐 아니라 사용자가 직접 편집한 경우에도 동일하게 동작. `patterns`에 glob 패턴(`*.java`, `*.jsx` 등)을 지정하여 특정 파일 유형에만 적용 가능. 파일 관련 이벤트 3종: `fileEdited`(저장 시), `fileCreated`(생성 시), `fileDeleted`(삭제 시). |

---

### 210. Kiro 생성 코드 리포트 Hook 에러 원인 확인

| 질의 | Kiro 생성 코드 리포트 Hook 에러 원인 확인 요청 |
|------|----------------------------------------|
| 결과 | `.kiro/hooks/kiro-generated-report.kiro.hook` 분석 완료. 에러 원인: `then.type`이 `runCommand`인데 `command` 값에 실제 셸 명령어가 아닌 자연어 프롬프트(한글 지시문)가 입력되어 있음. 셸이 이 텍스트를 명령어로 실행하려다 실패. 해결 방법: (1) `then.type`을 `askAgent`로 변경하고 `command`를 `prompt`로 변경 — 에이전트가 자연어 지시를 처리, (2) `then.type`을 `runCommand`로 유지하되 `command`에 실제 실행 가능한 스크립트(예: `node generate-guide.js`) 지정. |
